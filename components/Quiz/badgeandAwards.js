...
const QuizHistory = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchAttempts = async () => {
      try {
        const res = await fetch(`http://192.168.0.109:4001/api/v1/attempt/user/${userId}`);
    ...
    <View style={styles.container}>
      ...
      {highestBadge ? (
        <View style={styles.badgeContainer}>
          <Image source={{ uri: `http://192.168.0.109:4001/${highestBadge.media}` }} style={styles.badgeImage} />
